...
public class RecipeSmeltingFunction extends VarArgFunction {
    @Override
    public Varargs invoke(Varargs args) {
        LuaTable table = args.arg(1).checktable();
        String result = table.get("1").checkjstring();
        String ingredientItemId = table.get("2").checkjstring();
        ...
        RecipeBuilder.SmeltingRecipeBuilder recipeBuilder = RecipeBuilder.smelting(result);
        recipeBuilder.ingredient(ingredientItemId);

        if (cookingTime > 0) recipeBuilder.time(cookingTime);
        if (experience > 0.0) recipeBuilder.experience((float) experience);

        JsonObject recipeJson = recipeBuilder.build();

        String path = resultString.replace(":", "_") + "_smelting_" + Integer.toHexString(recipeJson.hashCode());
